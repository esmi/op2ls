...
toolbar_list_table1_events(){
cat <<-EOF
        <%if aryPerm(1) = true then %>
            <td nowrap ${_TBLIST_ADDNEW_STYLE}><span onclick='AddNew();' class='clsHref' language='jscript' 
...
cat <<-EOF
        <td nowrap ${_TBLIST_SEARCHATTACH_STYLE}><span onclick='SearchAttach();' class='clsHref' language='jscript'
            onmouseover='this.className="clsHrefOver";' onmouseout='this.className="clsHref";'
